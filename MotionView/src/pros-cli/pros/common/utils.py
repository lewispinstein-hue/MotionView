...
def retries(func, retry: int = 3):
    ...
    def retries_wrapper(*args, n_retries: int = retry, **kwargs):
        try:
            ...
            # Radio sync headers often fail under high load. 
            # Give the V5 hardware 50ms to clear its buffers.
            if n_retries > 0:
                time.sleep(0.05)
                return retries_wrapper(*args, n_retries=n_retries - 1, **kwargs)
            raise e
        except TypeError as e:
            # Port was likely closed by another thread; retrying is futile.
            raise e
