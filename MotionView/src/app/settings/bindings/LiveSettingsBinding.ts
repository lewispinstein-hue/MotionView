...
export class LiveSettingsBinding {
  ...
  bind(): void {
    ...
    this.live.events.projectChanged.subscribe(() => this.settings.update({
      prosDir: this.live.project.path },
      "system"
    ));
    this.live.events.preferencesChanged.subscribe(() => this.settings.update({
      refreshIntervalMs: String(this.live.preferences.refreshIntervalMs) },
      "system"
    ));
  }
  applyAll(): void {
