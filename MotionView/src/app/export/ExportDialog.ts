...
export class ExportDialog {
  ...
  private render(): void {
    ...
    this.dom.customWrap.hidden = location !== "custom"; this.dom.filenameHint.textContent = this.dom.filename.value ? `File: ${filename(this.dom.filename.value)}.json` : "Only letters, numbers, spaces, dashes, and underscores are kept."; this.dom.customHint.textContent = location === "project" ? `Exports to ${this.projectDirectory || "the PROS project MotionView-Routes folder"}.` : "Enter a folder path.";
  ...
  private async submit(): Promise<void> {
    ...
    try { const result = await this.service.export(request); this.#submitError = ""; this.dom.success.textContent = `Successfully exported ${request.pathName} to "${result.path}"`; this.dom.success.hidden = false; this.dom.validation.textContent = ""; this.app.core.status.setStatus(`Exported ${request.filenameBase}.json.`); }
