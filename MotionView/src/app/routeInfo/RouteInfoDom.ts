...
export class RouteInfoDom {
  readonly modal: HTMLElement; readonly close: HTMLButtonElement; readonly list: HTMLElement; readonly apply: HTMLButtonElement;
  private constructor(document: Document) { this.modal = requiredElement("routeInfoModal", HTMLElement, document); this.close = requiredElement("btnRouteInfoClose", HTMLButtonElement, document); this.list = requiredElement("routeInfoList", HTMLElement, document); this.apply = requiredElement("btnApplyRunSettings", HTMLButtonElement, document); }
  static from(documentRoot: Document = document): RouteInfoDom { return new RouteInfoDom(documentRoot); }
}
